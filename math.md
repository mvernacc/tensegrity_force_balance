...
Calculate the rank of $M_+$. If $\mathrm{rank}(M_+) = \mathrm{rank}(M)$,
then $\vec{dm}$ is in the subspace spanned by the columns of $M$,
i.e. is a valid degree of freedom of the system.
If $\mathrm{rank}(M_+) = \mathrm{rank}(M) + 1$, then $\vec{dm}$ was not in the null space,
and the provided rotation and translation are not allowed by the constraints.
Note that $\mathrm{rank}(M)$ is the number of columns of $M$ and is the number of
(orthogonal) degrees of freedom of the system.
